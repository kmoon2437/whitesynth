...
| `0x26` | Pan | L63(1) - Center(64) - R63(127) | Audio channel에 따라 변화(Left: L63(0),Right: R63(127),Both: Center(64)) | 이펙트를 적용하는 쪽의 pan을 조절하며, Both이면 평범한 pan과 동일하게 작동한다. |
...
| `0x2F` | Multi effects off | (값 없음) | (값 없음) | 모든 채널에 대해 이 multi effect를 끈다. 파라미터 이름을 뭘로 정할지 모르겠어서 이렇게 했다. |
