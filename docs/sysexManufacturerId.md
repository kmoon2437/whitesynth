# Sysex용 제조사 ID
sysex 메세지의 가장 첫 바이트에는 일반적으로 이 메세지 형식이 어느 회사에서 만들어졌는지를 의미하는 "제조사(manufacturer: 생산자) ID"라는 게 들어간다.
가장 첫 바이트에 들어가므로 길이는 1바이트, 즉 최대 갯수는 127개이다.(midi 메세지의 데이터 값에서는 가장 첫 바이트를 무시하므로) 그러나 `0x60` - `0x7f` 범위는 별도 사용을 위해 예약되어 있으므로 사실상 최대 95개의 회사만이 고유한 제조사 ID를 가질 수 있다.
