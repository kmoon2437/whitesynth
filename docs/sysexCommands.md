...
GS Mode로 설정되는 경우 작동 방식이 아래와 같이 변경된다.
- `CC#77`(Vibrato depth)의 값의 범위가 -64 - 0 - +63(공식 = x + 64) (으)로 바뀐다. 자세한 것은 <./controlChanges.md> 를 참조하라.
...
- `F0` = 이 메세지가 Sysex 메세지임을 의미함
- `41` = 제조사 ID (Roland)
- `10` = Sysex 채널 (사실상 무쓸모지만 가급적 `10`으로 놔둘 것을 권장)
- `42` = 모델 ID (GS)
- `12` = 명령 ID
