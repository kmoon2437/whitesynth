...
impl Envelope {
    ...
    pub fn process(&mut self, input: f64) -> f64 {
        ...
        let mut level = if self.decay_time == 0.0 {
            1.0
        } else {
            self.sustain_level
        };
        ...
        } else if self.decay_time != 0.0
            && self.attack_time <= current_ms
            && current_ms <= (self.attack_time + self.hold_time)
        {
            level = 1.0;
        } else if self.decay_time != 0.0
            && (self.attack_time + self.hold_time) <= current_ms
            && current_ms <= (self.attack_time + self.hold_time + self.decay_time)
        {
            // decay time이 0이 아닌 경우 음량이 sustain level까지 decay time 동안 내려가도록 함
            level = 1.0
                - (1.0 - self.sustain_level) * (current_ms - (self.attack_time + self.hold_time))
                    / self.decay_time;
        }
        ...
        if self.released {
            if self.release_time == 0.0 {
                ...
                let after_released_tick_ms =
                    (self.current_tick - self.released_tick) / self.sample_rate * 1000.0;
