...
impl Delay {
    ...
    pub fn set_time(&mut self, time: f64) {
        self.ring_buf.set_interval(
            (self.sample_rate * (time.max(0.0).min(MAX_INTERVAL_SEC_F64) / 1000.0)) as usize
        );
    }
    ...
    pub fn process(&mut self, input: f64) -> f64 {
        ...
        self.ring_buf.write(util::synth::mix_two_samples(
            input,
            self.feedback * self.ring_buf.read(0)
        ));
        self.ring_buf.next();
        return output;
    }
