...
impl SF2 {
    pub fn new<T: std::io::Read + std::io::Seek>(
        stream: &mut T
                ...
                if child_type == "INFO" {
                    ...
                    match chunk.id().as_str() {
                        "shdr" => {
                            shdr = contents;
                            if shdr.len() % 46 != 0 {
                                return Err(SF2Error::new_box("invalid length of chunk 'shdr'"));
                            }
                        }
                        "inst" => {
                            inst = contents;
                            if inst.len() % 22 != 0 {
                                return Err(SF2Error::new_box("invalid length of chunk 'inst'"));
                            }
                        }
                        "ibag" => {
                            ibag = contents;
                            if ibag.len() % 4 != 0 {
                                return Err(SF2Error::new_box("invalid length of chunk 'ibag'"));
                            }
                        }
                        "imod" => {
                            imod = contents;
                            if imod.len() % 10 != 0 {
                                return Err(SF2Error::new_box("invalid length of chunk 'imod'"));
                            }
                        }
                        "igen" => {
                            igen = contents;
                            if igen.len() % 4 != 0 {
                                return Err(SF2Error::new_box("invalid length of chunk 'igen'"));
                            }
                        }
                        "phdr" => {
                            phdr = contents;
                            if phdr.len() % 38 != 0 {
                                return Err(SF2Error::new_box("invalid length of chunk 'phdr'"));
                            }
                        }
                        "pbag" => {
                            pbag = contents;
                            if pbag.len() % 4 != 0 {
                                return Err(SF2Error::new_box("invalid length of chunk 'pbag'"));
                            }
                        }
                        "pmod" => {
                            pmod = contents;
                            if pmod.len() % 10 != 0 {
                                return Err(SF2Error::new_box("invalid length of chunk 'pmod'"));
                            }
                        }
                        "pgen" => {
                            pgen = contents;
                            if pgen.len() % 4 != 0 {
                                return Err(SF2Error::new_box("invalid length of chunk 'pgen'"));
                            }
                        }
                        &_ => {}
                    }
